const GOL = (tileSize: number, fps: number) => {
    const main = () => {
        const container = $('#gol');
    ...
    const update = (grid: number[][]) => {
        // make a deep copy of the grid
        const nextGrid = <number[][]>JSON.parse(JSON.stringify(grid));
    ...
    const countNeighbors = (grid: number[][], row: number, column: number) => {
        let count = 0;
        for (let y = -1; y <= 1; y++) {
            for (let x = -1; x <= 1; x++) {
                if (x == 0 && y == 0) continue;
                if (!grid[y + row] || !grid[y + row][x + column]) continue;
                if (grid[y + row][x + column] == 1) count++;
            }
        }
        return count;
    };

    const draw = (grid: number[][], canvas: HTMLCanvasElement) => {
    ...
    const mouseRandomize = (grid: number[][], radius: number) => {
        for (let y = 0; y < grid.length; y++) {
            for (let x = 0; x < grid[y].length; x++) {
